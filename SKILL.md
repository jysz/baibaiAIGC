---
name: baibaiaigc
description: '对中文论文、摘要、课程作业、技术文档进行多轮降 AIGC 改写。用户提到降AIGC、论文去AI味、降低AI痕迹、人性化改写论文、按顺序使用多个提示词改写文本、多轮改写时必须使用。本 skill 必须严格按 1-2-3 顺序依次使用 prompts/baibaiAIGC1.md、prompts/baibaiAIGC2.md、prompts/baibaiAIGC3.md，每次调用只执行一轮改写，依靠“降 AIGC 记录”跨对话串联轮次，并在最终根据 checklist.md 做检查和评分。'
user-invocable: true
---
...
- 三轮顺序固定为：`prompts/baibaiAIGC1.md` -> `prompts/baibaiAIGC2.md` -> `prompts/baibaiAIGC3.md`，禁止跳轮或逆序。
...
- 在每次调用本 skill、开始改写之前，必须先读取这份记录：
  ...
  - 如果当前文档已经完成第 1 轮和第 2 轮但未完成第 3 轮，则本次执行第 3 轮，使用 `prompts/baibaiAIGC3.md`，并以上一轮输出的文件作为当前输入。
...
当“降 AIGC 记录”中显示当前文档已完成第 1 轮和第 2 轮但尚未完成第 3 轮时，本次执行第 3 轮。读取工作区文件 `prompts/baibaiAIGC3.md`。
