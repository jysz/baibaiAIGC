---
name: baibaiAIGC
description: '对中文论文、摘要、课程作业、技术文档进行多轮降 AIGC 改写。用户提到降AIGC、论文去AI味、降低AI痕迹、人性化改写论文、按顺序使用多个提示词改写文本、多轮改写时必须使用。本 skill 必须严格按 1-2 顺序依次使用 prompts/baibaiAIGC1.md、prompts/baibaiAIGC2.md，每次调用只执行一轮改写，依靠“降 AIGC 记录”跨对话串联轮次，并在最终根据 checklist.md 做检查和评分。当前项目已屏蔽第 3 轮。'
user-invocable: true
---
...
- 多轮降 AIGC 必须严格按顺序执行两轮改写，但每次调用本 skill 只执行其中一轮。
- 两轮顺序固定为：`prompts/baibaiAIGC1.md` -> `prompts/baibaiAIGC2.md`，禁止跳轮或逆序。
...
为了避免单次对话上下文过长，同时在多个对话之间保持两轮顺序的一致性，本 skill 需要维护一份“降 AIGC 记录”，用于跨对话记住每篇文档已经执行到第几轮、使用了哪个 prompt，以及对应输出文件路径。
